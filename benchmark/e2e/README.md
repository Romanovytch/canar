...
Select one or more benchmark YAML files and a retrieval-metric cutoff from the
command line. Relative config paths are resolved from the current working
directory:

```bash
MEASURE_RESOURCES=1 python e2e/eval_e2e.py \
  --config config.dico.yaml \
  --config config.utilitr.yaml \
  --retrieval-k 5
```

Put the collection in the matching YAML:

```yaml
environment:
  collection: dicovar_sum
```

The LLM judge, generation settings, embedding client, token counter and
`MEASURE_RESOURCES` are process-wide for the full CLI invocation. The
`--retrieval-k` option changes only metric computation, not product retrieval or
reranker result counts.
...
- `--config PATH` (default `config.yaml`) — benchmark run/dataset/profile YAML;
  repeat it to run several configs in one batch.
